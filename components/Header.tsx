        ...
        <div className='flex items-center justify-between'>
            ...
            <div className='hidden lg:flex items-center gap-4'>
                {links.map((link , index)=>{
                    return <motion.div className='text-white' initial={{y: -100 , opacity: 0}} animate={{y: 0 , opacity: 1}} viewport={{once: true}} transition={{duration: 0.7 , ease: "linear" , delay: index * 0.2}}  key={index}>
    ...
    <div className={`${showNav ? "translate-y-0" : "-translate-y-full"} lg:hidden transition-transform duration-1000 fixed z-50 w-full h-screen flex flex-col justify-center items-center gap-4 bg-black top-0 left-0`}>
        ...
        <AnimatePresence>
            {showNav && links.map((link , index)=>{
                return <ScrollLink onClick={()=> setShowNav(false)} className='text-[24px] text-white cursor-pointer w-fit relative before:absolute before:w-0 before:h-[2px] before:-bottom-1 before:left-0 before:bg-main hover:before:w-full before:transition-all before:duration-500' key={index} to={link.href}>{getLetter(link.label)}</ScrollLink> 
            })}
        </AnimatePresence>
    </div>
    </>
